...
const validateHttpsUrl = (value, fieldPath) => {
  if (typeof value !== "string" || !value.startsWith(HTTPS_SCHEME_PREFIX) || CONTROL_OR_WHITESPACE_PATTERN.test(value) || value.includes("\\")) {
    fail("provenance.invalid_source", fieldPath);
  }
  const rawAuthority = value.slice(HTTPS_SCHEME_PREFIX.length).split(/[/?#]/u)[0];
